...
const Stoplazma = "#36393e";
const StoplazmaDogru = "#22BF41";
const StoplazmaHata = "#f30707";
...
.setThumbnail()
.addField(`Stoplazma | Teşekkürler`, `**Selamlar, Ben Muzaffer Karaman (Stoplazma'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Stoplazma | Prefix`, `**Stoplazma Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Stoplazma | Nasıl Kullanılır?`, `**Stoplazma botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Stoplazma | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.addField(`Stoplazma | NOT`, `**Bazı Emojiler Çalışmamaktadır Onlar İçin Kusura Bakmayın Yakında Düzeltilicektir**`)
.setFooter(`Stoplazma | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
.then(function() {
console.log('Stoplazma Başarıyla Tokene Ulaşdı Giriş Yapılıyor')
}, function(err) {
console.log("Stoplazma Girerken Bir Hata İle Karşılaşdı Lütfen Tokeni Kontrol Et :d.")
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!856585340587868191>`) return msg.channel.send(`> **Stoplazma | Prefix**\n\n> <a:onay2:831854235737129010>  **Sanırım beni etiketlediniz.**\n > <a:onay2:831854235737129010>  Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:sorgu:856594969740836895>  <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Stoplazma Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffffff")
    ...
    .setDescription(`<:ban:856595451733082163>  **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Stoplazma Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:as:831866956142477348>  Aleyküm Selam, Hoşgeldin. <a:as:831866956142477348>** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:gir:831852497123016754>  `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:k_:831852496787734578>  `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:gir:831852497123016754>  `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
