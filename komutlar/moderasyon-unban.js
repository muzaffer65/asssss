...
let prefix = await db.fetch(`prefix.${message.guild.id}`) || fynx.prefix;
 ...
 if (isNaN(user)) return message.channel.send('<:a_:821738957997211659>  **Lütfen Banını Açmak İstediğiniz Üyeninin ID sini Girin**');
  ...
  .setColor("#ffffff")
  .addField('<a:onay4:856597033879601184>  İşlem', 'Ban Kaldırma')
  .addField('<a:onay4:856597033879601184>  Banı Açılan Üye', `(${user})`)
  .addField('<a:onay4:856597033879601184>  Banı Açan Yetkili', `${message.author.username}#${message.author.discriminator}`)
  .addField('<a:onay4:856597033879601184>  Banı Açma Sebebi', "```" + reason + "```")
  modlog.send(embed);
  ...
  .setColor("#ffffff")
  .setDescription(`<a:onay4:856597033879601184>  Belirtiğiniz İD'nin Banı Açıldı`)
