...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`<a:hata1:831855888007954453>  **Modlog Kanalı Zaten ayarlı değil**`);
    db.delete(`log_${message.guild.id}`)
   message.channel.send(`<a:onay4:856597033879601184>  **ModLog Kanalı başarıyla sıfırlandı.**`);
  
    return
  }
  
if (!logk) return message.channel.send(`<a:hata1:831855888007954453>  **Bir modlog kanalı belirtmelisin.**`);
